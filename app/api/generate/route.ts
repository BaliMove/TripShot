...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (customBgBase64) {
      ...
      prompt = `(masterpiece, best quality:1.2), RAW unretouched photo, 8k uhd, shot on Canon EOS R5 (85mm f/2.2 for single portrait / 35mm f/4.0 for multi-person group) seamlessly integrating ALL person(s) present in Image 1 (Indoor/Casual Portrait) naturally into the scenic/extreme destination backdrop in Image 2 (Scenic/Extreme Landscape Background).
...
5. FASHION & STYLING: Elegantly adapt the person's clothing to match travel/outdoor adventure aesthetic while maintaining true personal identity.${customPromptAddon}${fixAddon} ${NO_TEXT_INSTRUCTION} CRITICAL NEGATIVE: (plastic skin, waxy skin, airbrushed, smooth skin, poreless skin:1.4), (matte skin, powdery skin, flawless skin:1.3), beauty filter, glam, cgi, 3d render, cartoon, painting, illustration, drawing, unreal engine, ring-light flat lighting, front flash, overexposed highlights, dead eyes, bad anatomy, deformed hands, lowres, watermark, halo artifact around head, hard cutout edges, mismatched lighting, unnatural seams, skin tone boundary mismatch, blurry borders, oversmoothed skin, cartoonish outline, deformed facial features, mutated eyes, asymmetrical jaw, identity drift, face blending, duplicated face, identical features across multiple people, swapped identities, merged facial attributes, oversized head, giant head, missing people, dropped members, fake floating cutout.`;
      ...
      if (facePreserveMode) {
        prompt += ` CRITICAL 100% REAL-FACE & PROPORTIONAL HARMONY DIRECTIVE: Reconstruct the subject's authentic 1:1 real face from Image 1 (eyes, double eyelids, nose, smile, eyeglasses) with id_weight: 0.999. Maintain realistic natural head-to-body ratio (1:7 to 1:8, Waist-Up Medium Shot 45%-60% frame height, do not zoom into giant head) with authentic micro-pores, fine peach fuzz, and sub-surface scattering, harmoniously integrated with the breathtaking travel landscape in the background.`;
      }
