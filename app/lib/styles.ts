...
export function getTravelPrompt(destination: string): string {
  const SMART_GEAR =
    "the traveler(s) from Image 1 looking naturally stunning in stylish chic vacation attire, with natural wide-open large expressive eyes, clear double eyelids, authentic smile, and 100% genuine facial identity from Image 1, posed gracefully in an environmental knee-up travel shot (occupying 33% to 40% of the vertical frame for high-resolution facial clarity) beautifully harmonized with the expansive landmark panorama in background";
