...
export function getTravelPrompt(destination: string): string {
  const SMART_GEAR =
    "the traveler(s) from Image 1 looking naturally stunning in stylish chic vacation attire (elegant trench coat, chic jacket, or stylish knitwear, completely replacing any casual indoor clothes), posed gracefully in an environmental full-body or knee-up travel shot (occupying 25% to 32% of the vertical frame) beautifully harmonized with the expansive landmark panorama in background";
