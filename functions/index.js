...
app.post("/api/generate", async (req, res) => {
  ...
  try {
    ...
    if (customBgBase64) {
      ...
      finalPrompt = `(masterpiece, best quality:1.2), RAW unretouched photo, 8k uhd, shot on Canon EOS R5 (85mm f/2.2 for single portrait / 35mm f/4.0 for multi-person group) seamlessly integrating ALL person(s) present in Image 1 (Indoor/Casual Portrait) naturally into the scenic/extreme destination backdrop in Image 2 (Scenic/Extreme Landscape Background).
...
4. MULTI-PERSON REGIONAL SPLIT [SEP]: If multiple individuals (e.g. 2, 4, or a group) are present in Image 1, detect and include ALL of them together [Person 1] [SEP] [Person 2] naturally with balanced group framing.`;
      ...
      finalPrompt = `(masterpiece, best quality:1.2), RAW unretouched photo, 8k uhd, shot on Canon EOS R5 (85mm f/2.2 for single / 35mm f/4.0 for multi-person group) travel photography masterpiece seamlessly integrating ALL person(s) present in Image 1 into the scene: ${basePrompt}.
CRITICAL MANDATORY INSTRUCTIONS:
