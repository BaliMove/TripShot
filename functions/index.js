...
app.post("/api/generate", async (req, res) => {
  ...
  try {
    ...
    if (customBgBase64) {
      ...
      finalPrompt = `(masterpiece, best quality:1.2), RAW unretouched photo, 8k uhd, shot on Canon EOS R5 (35mm-50mm environmental prime lens at f/2.8) seamlessly integrating ALL person(s) present in Image 1 (Indoor/Casual Portrait) naturally into the scenic/extreme destination backdrop in Image 2 (Scenic/Extreme Landscape Background).
CRITICAL MANDATORY INSTRUCTIONS:
1. 100% UNTOUCHED REAL FACE ID LOCK (IP-Adapter FaceID Fidelity, id_weight: 0.999): Every person's authentic facial bone structure, eyes, double eyelids, nose bridge, mouth, smile, teeth, expressions, eyeglasses, hairline, and skin texture with microscopic visible pores, fine vellus peach fuzz, and natural sub-surface scattering (SSS) from Image 1 MUST be preserved 100% authentically with zero hallucination. Absolutely NO plastic or waxy skin.
2. GOLDEN-RATIO ENVIRONMENTAL TRAVEL COMPOSITION: Frame the person(s) naturally in a Medium-to-Full or Waist-to-Knee pose occupying 35% to 45% of the total vertical frame height. The majestic landscape/landmark backdrop from Image 2 MUST occupy 55% to 65% of the frame with full panoramic grandeur and deep perspective. Strictly DO NOT zoom in on the head into an oversized ID headshot or tight chest-up close-up that blocks the scenery.
3. EDGE INPAINTING & FEATHER BLENDING: Extreme microscopic skin texture with visible pores, subtle flyaway hair strands breaking contour, seamless edge blending, matching ambient lighting, directional rim light, and natural contact shadows so the subjects look 100% organically photographed at the real location on Kodak Portra 400 film.
